...
class crane:
  ...
  def time_check (list1,list2): #충돌시 밀어서 시간구하는 함수
    add=0
    compare=0
    is_collision = True
  ...
  def cluster(order1, order2): #cluster내의 전체 trajectory 구하는 함수
      result = []
      cluster=[]
      x1=[]
      x2=[]
      ...
      t_cluster,add =time_check(order1,order2) # 전체소요시간과 add 구하기
  ...
  def get_trans(i_,j_,i,j,cluster_set): # cluster간 transition time 구하는 함수
  ...
  def get_init(i,j,cluster_set): #crane의 초기이동 시간 구하는 함수 
  ...
  def repeat(i, j, left, temp_dict,init_dict,trans_dict,cluster_set): #dp
      if len(left) == 2:
          ...
          for j_ in left_:
              if i_ != j_:
                  if (i_, j_, str(left_)) in temp_dict:
                      ...
                      min_time_, _, _ = repeat(i_, j_, left_, temp_dict,init_dict,trans_dict,cluster_set)
  ...
  def full_traj(list1, list2, add, result):
      t1=[]
      t2=[]
      x1=[]
      x2=[]
  ...
  def full_traj2(list3 ,list4 ,list1, list2, add, result ,tran):
                #앞클  #앞클  #뒷클  #뒷클
      t1=[]
      t2=[]
      x1=[]
  ...
  def plot_all(test,num):
    orders=[]
    cluster_set={}
    for index, row in test.iterrows():
        orders.append([row[1], row[2], row[0]])
    perm=list(permutations(orders,2))

    for i in range(len(perm)):
        cluster_set[(perm[i][0][2],perm[i][1][2])]=(cluster(perm[i][0],perm[i][1])) #cluster_set형성

    trans_dict={}
    for i in perm: # perm은 order 
        for k in perm:
            temp_set=set()
            ...
            if len(temp_set)==4: 
                trans_dict[(k[0][2],k[1][2]),(i[0][2],i[1][2])]=get_trans(k[0][2],k[1][2],i[0][2],i[1][2],cluster_set)


    init_dict={}
    for i in perm: 
        init_dict[(i[0][2],i[1][2])]=get_init(i[0][2],i[1][2],cluster_set)
    left=set(range(len(orders)))
    temp_dict={}
    min_times = []
    orders = []
    for a in left:
        for b in left:
            if a!= b:
                min_time, temp_order, temp_dict = repeat(a, b, left, temp_dict,init_dict,trans_dict,cluster_set)
    ...
    idx = '\n'.join([str(row) for row in min_order])
    ...
    for k in range(len(min_order)):
        if k ==0:
            temp=full_traj(cluster_set[min_order[k]][0], cluster_set[min_order[k]][1], cluster_set[min_order[k]][2],cluster_set[min_order[k]][3]) #cluster별 전체 traj 출력
                            #우선순위1인 order              #우선순위2인 order             # add                          #time
        else:
            temp=full_traj2(cluster_set[min_order[k-1]][0],cluster_set[min_order[k-1]][1],cluster_set[min_order[k]][0], cluster_set[min_order[k]][1], cluster_set[min_order[k]][2],cluster_set[min_order[k]][3],trans_dict[min_order[k-1],min_order[k]]) #cluster별 전체 traj 출력
    ...
    init_time=init_dict[min_order[0]]
    ...
    for k in range(1,len(temp_x1)):
        if trans_dict[min_order[k-1],min_order[k]]> abs(temp_x1[k][0] - x1[-1]): # 2의 transition time 이 더 길다. 
            for i in range(trans_dict[min_order[k-1],min_order[k]]- abs(temp_x1[k][0] - x1[-1])):
                x1.append(x1[-1])
        if trans_dict[min_order[k-1],min_order[k]]> abs(temp_x2[k][0] - x2[-1]):
            for i in range(trans_dict[min_order[k-1],min_order[k]]- abs(temp_x2[k][0] - x2[-1])):
                x2.append(x2[-1])
        trans1=0        
...
if __name__ == '__main__':
    for k in range(1,31):
        test= pd.read_excel('order10_1.xlsx', sheet_name=f'test{k}')
        crane_t = crane() 
        crane_t.plot_all(test,k)